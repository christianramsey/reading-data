...
for file in glob.glob("data/trajectories.csv*"):
...
classifier.train(
    input_fn=lambda: my_input_fn(filepaths,
                                 True, batch_size=220,
                                 repeat_count=1))

accuracy_score = classifier.evaluate(input_fn=lambda: \
    my_input_fn(filepaths, False, repeat_count=1, batch_size=1))["accuracy"]
...
for pred_dict, expec in zip(predictions_score, predict_x):
